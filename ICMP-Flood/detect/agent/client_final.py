...
def count_icmp_packets(packet, local_ip, icmp_counter):
    if IP in packet and ICMP in packet:
        ip_source = packet[IP].src
        if ip_source != local_ip:
            icmp_counter[ip_source] += 1

def print_statistics(local_ip, icmp_counter, time_counter):
    while True:
...
statistics_thread = threading.Thread(target=print_statistics, args=(local_ip, icmp_counter, time_counter))
...
sniff(prn=lambda pkt: count_icmp_packets(pkt, local_ip, icmp_counter), store=0)
